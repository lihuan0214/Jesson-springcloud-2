...
import com.xiangxue.jack.MicroWebApplicationDemo;
...
@SpringBootTest(classes = MicroWebApplicationDemo.class)
@WebAppConfiguration
public class MyTest {
        ...
        for (Integer i = 0; i < count; i++) {
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    logger.info(Thread.currentThread().getName() + "==>" + userService.listUser());
                }
            }).start();
            cdl.countDown();
        }
    ...
    * */
    ...
    public void test1() {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder().uri(new URI("/user/listUser")).build();
